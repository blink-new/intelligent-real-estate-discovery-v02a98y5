...
export function ReActConversationalSearchWithMemory({ onPropertiesFound, onClose }: ReActConversationalSearchProps) {
  ...
  // Initialize user and memory system
  useEffect(() => {
    const initializeMemory = async () => {
      try {
        ...
        if (currentUser) {
          ...
          await memoryManager.initializeSession(currentUser.id)
          ...
          const history = await memoryManager.getConversationHistory(5)
          setConversationHistory(history)
          
          // Load search history
          const searches = await memoryManager.getSearchHistory(10)
          ...
          if (context.length > 0) {
            ...
            const displayMessages = context
              ...
              .map(msg => ({
                ...
                reactResponse: msg.metadata?.reasoning ? {
            ...
            // Set welcome message with personalized context
  ...
  const getPersonalizedWelcomeMessage = (preferences: any): ReActMessage => {
    let content = "Hi! I'm your AI real estate assistant powered by the **ReAct Framework** and **Gemini AI**. I use step-by-step reasoning and real-time tools to help you find the perfect property in Nepal.\n\n🧠 **ReAct Framework** - Transparent reasoning process\n🔍 **Real-time Search** - Current market data via Tavily API\n🗺️ **Maps Integration** - Location insights via Google Maps\n📊 **Market Analysis** - Investment insights and ROI calculations\n🏠 **Property Database** - Comprehensive listings search\n❓ **Smart Clarification** - Asks the right questions when needed\n\nI'll show you my complete thinking process as I work through your request."

    if (preferences) {
      content += "\n\n**🎯 Your Profile:**"
      if (preferences.propertyType?.length) {
        content += `\n• Interested in: ${preferences.propertyType.join(', ')}`
      }
      if (preferences.priceRange) {
        content += `\n• Budget: NPR ${preferences.priceRange.min?.toLocaleString()} - ${preferences.priceRange.max?.toLocaleString()}`
      }
      if (preferences.locations?.length) {
        content += `\n• Preferred areas: ${preferences.locations.join(', ')}`
      }
      if (preferences.searchHistory?.length) {
        content += `\n• Recent searches: ${preferences.searchHistory.slice(-3).join(', ')}`
      }
    }
  ...
  const getDefaultWelcomeMessage = (): ReActMessage => ({
    id: '1',
    role: 'assistant',
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Process query with user ID for memory integration
      const reactResponse = await reactAgent.processQuery(inputValue, user?.id)
      ...
      if (user?.id) {
        const updatedSearches = await memoryManager.getSearchHistory(10)
        setSearchHistory(updatedSearches)
        
        // Refresh user preferences in case they were updated
        const updatedPreferences = await memoryManager.getUserPreferences()
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="p-4 border-t bg-white">
            ...
            <div className="mt-2 text-xs text-muted-foreground text-center">
              Powered by ReAct Framework • Gemini AI • Real-time Tools • {user ? 'Memory-Enhanced' : 'Transparent Reasoning'}
            </div>
          </div>
        </div>

        {/* Memory Panel */}
        {showMemoryPanel && user && (
          <div className="w-80 border-l bg-muted/20">
            ...
            <Tabs defaultValue="preferences" className="h-full">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {userPreferences.propertyType?.map((type: string) => (
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {userPreferences.priceRange ? 
                          `NPR ${userPreferences.priceRange.min?.toLocaleString()} - ${userPreferences.priceRange.max?.toLocaleString()}` :
                          'Not specified'
                        }
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {userPreferences.locations?.map((location: string) => (
              ...
              <TabsContent value="searches" className="p-4">
                <div className="space-y-2">
                  {searchHistory.length > 0 ? (
                    searchHistory.map((search, index) => (
                      <Card key={index} className="p-2 cursor-pointer hover:bg-muted/50" onClick={() => setInputValue(search)}>
